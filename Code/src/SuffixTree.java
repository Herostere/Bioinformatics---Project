...
public class SuffixTree {
    ...
    public static EdgeSF[] EDGES = new EdgeSF[HASH_TABLE_SIZE];
    ...
    public static NodeSF[] NODES = new NodeSF[MAX_LENGTH * 2];
    ...
    public static void addPrefix(Suffix active, int last_char_index) {
        ...
        for (; ;) {
            ...
            if (active.explicit()) {
                edge = EdgeSF.find(active.origin_node, T[last_char_index]);
                if (edge.start_node != -1)
                    break;
            } else {
                //implicit node, a little more complicated
                edge = EdgeSF.find(active.origin_node, T[active.first_char_index]);
            ...
            EdgeSF new_edge = new EdgeSF(last_char_index, N, parent_node);
            new_edge.insert();
